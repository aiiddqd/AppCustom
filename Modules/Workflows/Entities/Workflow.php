...
class Workflow extends Model
{
                    ...
                    case 'notification':
                        ...
                        foreach ($value as $user_id) {
                            if ($user_id == 'assignee') {
                                ...
                                $user = User::findNonDeleted($user_id);
                        ...
                        if ($users) {
                            \App\Jobs\SendNotificationToUsers::dispatch($users, $conversation, $conversation->getThreads())
                                ->onQueue('emails');
                            $executed = true;
                        }
                        break;
    ...
    public static function maybeProcessInBackground($workflow)
    {
        ...
        \Helper::backgroundAction('workflow.do_process', ['id' => $workflow->id]);
    }
    ...
    public static function canEditWorkflows($user = null)
    {
        if (!$user) {
            $user = auth()->user();
        }
                    ...
                    switch ($condition['type']) {
                        ...
                        case 'user':
                            if (empty($condition['value'])) {
                                ...
                                $user = User::nonDeleted()->where('id' , $condition['value'])->first();
                                if (!$user) {
                                    $has_error = true;
                                }
                            }
                    ...
                    switch ($action['type']) {
                        ...
                        case 'assign':
                            if (empty($action['value'])) {
                                ...
                                $user = User::nonDeleted()->where('id' , $action['value'])->first();
                                if (!$user) {
                                    $has_error = true;
                                }
                            }
                        ...
                        case 'notification':
                            if (empty($action['value'])) {
                                ...
                                if ($user_ids) {
                                    $count = User::nonDeleted()->whereIn('id' , $user_ids)->count();
